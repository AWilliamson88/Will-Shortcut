...
import { SHORTCUT_ACTIONS } from '../utils/shortcutActions';
...
export function SettingsModal({
	...
	const [activeTab, setActiveTab] = useState<'general' | 'applications' | 'shortcuts'>('general');
				...
				<div className="mb-2 border-b border-gray-700">
					<nav className="flex gap-2 text-sm">
						<button
							...
							className={`px-3 py-1 rounded-t ${activeTab === 'general'
						...
						<button
							...
							className={`px-3 py-1 rounded-t ${activeTab === 'applications'
									? 'bg-gray-900 text-white border border-b-transparent border-gray-700'
									: 'text-gray-400 hover:text-white'
								}`}
						>
						...
						<button
							...
							className={`px-3 py-1 rounded-t ${activeTab === 'shortcuts'
									? 'bg-gray-900 text-white border border-b-transparent border-gray-700'
									: 'text-gray-400 hover:text-white'
								}`}
						>
									...
									<tbody>
										{SHORTCUT_ACTIONS.map(action => (
											<tr key={action.id} className="border-t border-gray-800">
												<td className="px-2 py-1 text-gray-300">{action.label}</td>
												<td className="px-2 py-1">
													<KeyCaptureInput
														value={localSettings.keyboard_shortcuts[action.id]}
														onChange={value =>
															setLocalSettings(prev =>
																prev
																	? {
												...
												<td className="px-2 py-1 text-gray-400">{action.description}</td>
											</tr>
										))}
									</tbody>
								</table>
